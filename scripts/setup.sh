...
echo "  bash scripts/update.sh pull     # 工作前：拉取最新"
echo "  bash scripts/update.sh log      # 工作后：记录进度"
echo "  bash scripts/update.sh status   # 随时：查看动态"
echo ""
echo -e "${BLUE}On another device (cluster etc.):${NC}"
echo "  git clone <repo-url> ~/AGENTLab"
echo "  bash scripts/update.sh init $MEMBER_NAME    # 设置身份"
